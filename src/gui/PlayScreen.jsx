...
import gameNes from "../assets/theGame.nes";
...
export default class PlayScreen extends Component {
	...
	_onFileRun = async (file) => {
		try {
			// Завантаження файлу через Fetch API
			const response = await fetch(file);
			if (!response.ok) throw new Error(`Failed to load NES file: ${response.statusText}`);

			// Отримання вмісту файлу як ArrayBuffer
			const rom = await response.arrayBuffer();

			// Оновлення стану або передачу ROM до емулюючого компонента
			if (this.state.syncer) {
				this.state.syncer.updateRom(rom);
			} else {
				this._loadRom(rom);
				if (!shouldAutoStart) bus.emit("message", strings.pressStart);
			}
		} catch (error) {
			console.error("Error loading NES file:", error);
			bus.emit("message", strings.errors.failedToLoadRom);
		}
	};
